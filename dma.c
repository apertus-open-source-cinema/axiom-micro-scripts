...
int dma_s2mm_sync(unsigned int* dma_virtual_address, unsigned int * dest) {
    ...
    while(!(s2mm_status & 1<<12) || !(s2mm_status & 1<<1)){
    	...
    	printf("Destination memory block: "); memdump(dest, 256);
...
int main() {
    ...
    unsigned int* virtual_destination_address = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, 0x0c000000); // Memory map destination address
