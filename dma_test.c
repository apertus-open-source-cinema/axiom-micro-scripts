...
int dma_mm2s_sync(unsigned int* dma_virtual_address, unsigned int* dest) {
    unsigned int mm2s_status =  dma_get(dma_virtual_address, MM2S_STATUS_REGISTER);
    while(!(mm2s_status & 0x2)){
	printf("%d\n", mm2s_status & 0x2);
...
int main() {
    ...
    unsigned int* virtual_source_address  = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, src); // Memory map source address
    unsigned int* virtual_destination_address = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, dest); // Memory map destination address

    virtual_source_address[0]= 0x11223344; // Write random stuff to source block
    virtual_source_address[1]= 0x55667788; // Write random stuff to source block
    ...
    dma_mm2s_sync(virtual_address, virtual_destination_address);
