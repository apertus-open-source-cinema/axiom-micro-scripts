...
void dma_s2mm_status(unsigned int* dma_virtual_address) {
    ...
    printf(" length: %d", dma_get(dma_virtual_address, S2MM_LENGTH));
    printf("\n");
}
...
void transfer(unsigned int * virtual_address, size_t dest, size_t size) {
...
//    tlast_gen[0] = 0;
    dma_set(virtual_address, S2MM_LENGTH, size);
    dma_s2mm_status(virtual_address);
...
//    size_t size = 1 << 24;
    ...
    int32_t* tlast_gen = mmap(NULL, 65535, PROT_READ | PROT_WRITE, MAP_SHARED, dh, 0x41200000); // Memory map AXI Lite register block
    unsigned int* virtual_destination_address = mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, dh, dest_base); // Memory map destination address
...
//    memdump(virtual_destination_address, size);
    ...
    transfer(virtual_address, dest_base, blksize);

    for(int pos = 0; pos < size; pos = pos + blksize) {
	int dest = dest_base + pos;
	transfer(virtual_address, dest, blksize);
    } 
