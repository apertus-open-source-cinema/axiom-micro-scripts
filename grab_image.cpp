...
uint12_t get_shifted(volatile uint32_t * in, int offset, int lane = 0) {
    ...
    /*
    8 bits per lane per word;
    8 - offset in the first;
    min(8, 12 - (8 - offset)) in the second;
...
/*
    offset = 0;
    lane = 0;
    out.data |= in[0] & 0xff >> offset;
    out.data |= (in[1] & (0xff >> 4 + offset)) << 8 - offset;
    out.data |= (in[2] & (0xff >> (-4 +  offset))) << (16 - offset);

    out.data |= ((in[0] >> (lane * 8)) & 0xff) >> offset;
    out.data |= ((in[1] >> (lane * 8)) & (0xff >> (12 - (8 - offset)))) << (8 - offset);

    if((12 - 8 - (8 - offset)) > 0) {
        out.data |= ((in[2] >> (lane * 8)) & (0xff >> (12 - 8 - (8 - offset)))) << (8 + 8 - offset);
    }
    */
