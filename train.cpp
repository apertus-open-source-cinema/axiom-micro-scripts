...
struct delay_control {
    ...
    void set(int lane, int taps) {
        ...
        taps <<= lane * 5;
        //                                                         |-> mask of reset bit 
        taps = (((control_register[0] & ~mask) | (taps & mask)) & 0b011111111111111111111) | 0b1000000000000000000000;

        control_register[0] = (1 << 20) | taps;
        sleep_ms(50);
        control_register[0] = taps & ~0b1000000000000000000000;

        auto actually_set = control_register[2];
        if(actually_set != (taps & ~0b1000000000000000000000)) {
            fprintf(stderr, "failed to set delay taps, expected %d, got %d \n", taps & ~0b1000000000000000000000 , actually_set);
//            exit(-1);
        }
    }
};
